...
function AccountCard({
	...
	const limit =
		account.limit != null && account.limit > 0 ? account.limit : null;
	const hasLimit = limit != null;
	const hasCommitted = account.committed > 0;

	// Limite = cheque especial: só é consumido quando saldo é negativo
	const usedPct = hasLimit
		? account.currentBalance < 0
			? Math.min(100, (Math.abs(account.currentBalance) / limit) * 100)
			: 0
		: null;
	...
	return (
		<div className="relative group h-full">
			<Link
				...
				className="relative overflow-hidden rounded-2xl p-5 flex flex-col justify-between gap-3 shadow-md h-full transition-opacity hover:opacity-90 focus:outline-none focus-visible:ring-2 focus-visible:ring-white/50"
				style={{
					background: `linear-gradient(135deg, ${bank.bg} 0%, ${bank.accent} 100%)`,
					color: bank.text,
				}}
			>
				<div className="flex flex-col gap-3">
					...
					<div>
						...
						<p
							className={`text-2xl font-bold tabular-nums leading-none ${account.currentBalance < 0 ? "text-red-300" : ""}`}
						>
							{formatBRL(account.currentBalance)}
						</p>
					</div>

					{/* Barra de cheque especial */}
					{hasLimit && (
						<div className="space-y-1.5">
							...
							<div className="flex justify-between text-[10px] opacity-70">
								<span>Cheque especial: {formatBRL(limit ?? 0)}</span>
				...
				{/* Rodapé — sempre presente para altura uniforme */}
				<div className={hasCommitted ? "border-t border-white/20 pt-2.5 pb-6" : "pb-6"}>
					{hasCommitted && (
						<div>
...
function CreditCardCard({
	...
	const available =
		card.creditLimit != null ? card.creditLimit - card.currentBill : null;
	...
	return (
		<div className="relative group h-full">
			...
			>
				...
				{/* Rodapé — sempre presente para altura uniforme */}
				<div className="border-t border-white/20 pt-2.5 pb-6 flex items-end justify-between">
					{card.creditLimit != null ? (
						<div>
							<p className="text-[10px] opacity-60 uppercase tracking-wider">
								Disponível
			...
			<Dialog open={!!deletingAccount} onOpenChange={(open) => !open && setDeletingAccount(null)}>
				<DialogContent>
					...
					<p className="text-sm text-muted-foreground">
						...
						{deletingAccount?.institution ? ` — ${deletingAccount.institution}` : ""}
					</p>
