...
export function AccountForm({ account, onSave, onCancel }: Props) {
	const [form, setForm] = useState<FormState>(
		account ? accountToForm(account) : emptyForm(),
	);
	...
	async function handleSubmit(e: React.FormEvent) {
		...
		const res = await fetch(url, {
			...
			body: JSON.stringify({
				name: form.name,
				type: form.type,
				institution: isCash ? null : form.institution || null,
				currentBalance: parseFloat(form.currentBalance) || 0,
				limit: isCash ? null : form.limit ? parseFloat(form.limit) : null,
		...
		<form onSubmit={handleSubmit} className="space-y-4">
			<div className="space-y-2">
				...
				<Input
					id="acc-name"
					required
					placeholder="Ex: Nubank Conta, carteira"
					value={form.name}
			...
			<div className="grid grid-cols-2 gap-3">
				...
				<div className="space-y-2">
					...
					<Select
						value={form.currency}
						onValueChange={(v) => set("currency", v ?? "BRL")}
					>
						<SelectTrigger>
							<SelectValue />
						</SelectTrigger>
