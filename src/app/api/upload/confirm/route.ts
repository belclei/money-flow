...
const ConfirmSchema = z.object({
	...
	meta: z.object({
		...
		importType: z.enum(["fatura", "extrato"]).default("fatura"),
...
export async function POST(req: NextRequest) {
	...
	if (meta.importType === "extrato") {
		if (!meta.accountId) {
			return NextResponse.json(
				{ error: "accountId é obrigatório para extratos" },
				{ status: 400 },
			);
		}
		...
		return NextResponse.json(
			{ status: "ok", statement, unknownCardHolders: [] },
			{ status: 201 },
		);
	}

	// Fatura (credit card) flow
	...
	return NextResponse.json(
		{ status: "ok", invoice, unknownCardHolders },
		{ status: 201 },
	);
}
